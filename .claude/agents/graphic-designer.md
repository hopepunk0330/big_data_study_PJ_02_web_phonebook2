...
- **기본 아이콘과 비주얼 아이콘은 같은 두께를 쓰지 않는다**: 아이콘을 두 트랙으로 나눠 그린다.
  - **기본 아이콘(Basic/Utility)**: 로그아웃, 화면 전환 화살표(위/아래/이전/다음), 닫기, 펼치기/접기처럼 화면 어디서나 자주 반복되고 그 자체로 브랜드를 표현할 필요가 없는 아이콘. 얇고 중립적인 스트로크(브랜드의 굵은 잉크 아웃라인보다 가는 두께, 예: 1.5~2px)로 그려서 화면에서 튀지 않고 "배경 소음"처럼 자연스럽게 녹아든다.
  - **비주얼 아이콘(Visual/Feature)**: 카테고리, 알림처럼 브랜드 퍼스낼리티를 실제로 전달해야 하는 아이콘. 이 프로젝트의 굵은 잉크 아웃라인(3px)+브랜드색 평면 채색을 그대로 유지해 특징을 살린다.
  - 이 두 트랙을 같은 두께로 통일해버리면(예: 로그아웃 아이콘에 비주얼 아이콘과 똑같은 3px 굵은 선을 쓰면) 자주 등장하는 사소한 액션이 과하게 투박하고 무거워 보인다 — 실제로 이 문제가 있었다.
  - 세트 안에서는(같은 트랙끼리는) 스트로크 두께·모서리 반경을 동일하게 유지한다.
