...
판단 기준(디자인 원칙) :
- **아이콘 그리드 시스템**: 아이콘은 반드시 표준 그리드(16px 또는 24px 중 프로젝트 기준 하나로 통일)에 정렬한다.
- **기본 아이콘과 비주얼 아이콘은 같은 두께를 쓰지 않는다**: 아이콘을 두 트랙(Basic/Utility, Visual/Feature)으로 나눠 그린다. 트랙 판정 기준과 트랙별 크래프트 규칙(스트로크 두께 차등, 면색 유무 등)은 `@docs/harness/design-team/icon-craft-guide.md`를 따른다.
...
Figma 파일의 페이지 구조와 시안(Concept) 워크플로우는 `@docs/harness/design-team/figma-file-organization.md`를 따른다 (2-2번: 아이콘·일러스트레이션 담당 체계, 2-6번: 선제적 기본 구성 협업).

할 일 :
- `use_figma` 호출 전 반드시 `figma-use` 스킬을 로드한다.
- `docs/design/brand-guide.md`를 읽어 확정된 브랜드 톤·컬러·일러스트레이션 스타일 방향을 파악하고, 그 방향 안에서만 그린다.
