...
- 배경: `8:2`("브랜드컨셉") 페이지에 이미 존재하던 A(`8:3`)/네오팝 B(`8:4`, 이미 미채택 라벨)/콜라주 B(`28:2`, 라벨 없음)/C(`8:5`) 4개를 `get_metadata`+`get_screenshot`(base64)으로 먼저 직접 재확인한 뒤 진행. 신규 3개 프레임 제작: 병합 B `43:2`("Concept B — 브루탈 콜라주") x=3480, A 확장 `48:2`("Concept A — 확장 (정보 위계 강화)") x=0·y=2200, C 확장 `51:2`("Concept C — 확장 (히어로→시트 전환)") x=1720·y=2200 — 모두 기존 프레임과 좌표 겹침 없음.
