---
name: report-pdf
description: MD 문서를 docs/harness 템플릿 스타일의 HTML로 옮긴 뒤 PDF로 변환합니다 (doc-writer + pdf-maker 오케스트레이션).
---
...
1. **템플릿 쌍을 정한다.** 별도 지정이 없으면 기본값은 `docs/harness/report-style.css` + `docs/harness/report-format-guide.md`다(범용 절차 템플릿이라 하네스 쪽에 있다 — 프로젝트 고유 브랜드 색이 아니다). (나중에 발표용 등 다른 형식이 생기면 `docs/harness/slide-*` 같은 다른 쌍을 추가로 만들어 쓸 수 있다 — 사용자가 어떤 쌍을 쓸지 명시하면 그것을 따른다.) 내용 규칙(사람이 읽는 문서로 작성, 스타일 인라인, 구조 준수, 임의 요약 금지 등)은 doc-writer 자신의 역할 정의에 이미 있으므로 여기서 반복하지 않는다.
