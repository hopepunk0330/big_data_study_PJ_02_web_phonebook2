...
- **이 프로젝트가 아닌 다른 프로젝트 폴더는 무조건 읽기만 한다**(2026-08-24 신설) — 참고 자료 조회 목적이라도 다른 프로젝트 폴더 안의 파일을 쓰기(Write/Edit)·삭제·git 커밋하지 않는다. 그 자료를 근거로 새 작업(디자인 시안 등)을 만들 때도 결과물은 반드시 이 프로젝트 안이나 완전히 별도의 위치(예: 별도 Figma 파일)에 만들고, 원본 프로젝트 폴더 안에는 아무것도 쓰지 않는다.
  - **다른 프로젝트로 실제 전달(핸드오프)할 결과물이 생기면, `docs/하네스 협업/{대상 프로젝트 폴더명}/`에 스테이징한다**(2026-08-27 신설) — 대상 프로젝트가 읽기 전용 규칙 대상이라 그쪽 폴더에 직접 쓸 수 없으므로, 이 프로젝트 안에 대상 프로젝트명으로 하위 폴더를 만들어 전달할 파일(확정 산출물, 필요하면 그 산출물을 어떻게 쓰면 되는지 설명하는 메모)을 넣어두고, 실제로 그 폴더에 옮기는 건 사용자가 직접 한다. 예: 2026-08-27 mercari 아이콘 핸드오프 — `docs/하네스 협업/01-ML_mercari price_2608/`.
