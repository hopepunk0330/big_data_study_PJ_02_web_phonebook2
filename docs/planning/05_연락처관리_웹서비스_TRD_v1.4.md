...
> **변경 이력**
> - **v1.4 (2026-07-16)**: §4-4 `database.py` 예시의 `engine = create_engine(DATABASE_URL)`에 `pool_pre_ping=True` 옵션을 추가했습니다. DB 컨테이너(`pg-lab`)가 재시작되면 커넥션 풀에 남아있던 기존 커넥션이 끊어진 채로 재사용될 수 있는데, 이 옵션은 커넥션을 실제로 꺼내 쓰기 직전에 헬스체크를 실행해 죽은 커넥션을 자동으로 새 커넥션으로 교체합니다(SQLAlchemy 내장 동작, 추가 코드·라이브러리 불필요) — 01 문서 §7 NFR-01("어떤 요청에도 서버가 500을 응답하지 않을 것")을 지키기 위한 설정입니다. `get_db()`의 예외 처리(rollback)는 이 변경과 무관하므로 그대로 두었습니다(surgical — `create_engine()` 호출 한 줄만 대상). 근거: `docs/planning/tech-architecture.md` §1/§5(tech-architect 브리프, 사용자 승인 확정). §8-1에 FR-14/15 관련 Pydantic 스키마 4개(`FindPasswordIn`/`FindPasswordOut`/`ResetPasswordIn`/`MessageOut`)를 코드 블록으로 신규 추가했습니다(그동안 01 문서 §3-5 산문 설명만 있고 TRD에는 코드가 없었던 갭 해소).
> - **v1.3 (2026-07-16)**: §6 API 설계 요약 표 로그아웃(`POST /auth/logout`) 행의 "인증 필요" 표기가 06 테스트계획서(`TC-API-AUTH-11`, 무세션 호출도 항상 200)와 겉보기에 어긋나 보인다는 harness-auditor 지적에 따라 재검토했습니다. 텍스트상 실제 모순은 아니지만(해당 행 "주요 실패 코드" 칸이 처음부터 "-"), 06 문서 작성 중 이 지점에서 실제로 한 차례 오독이 발생해 planning-pl 검토를 거쳐서야 해소된 전례가 있어(01 문서 §3-1 FR-03·PRD §10 "01 우선" 규칙 근거) 재발 방지 차원에서 각주 `[^logout-auth]`를 신설, 354행 "인증 필요" 칸에 마커를 추가하고 표 바로 아래 각주 정의를 삽입했습니다. 표 구조·다른 행은 변경하지 않았습니다. 근거: tech-architect 판단 브리프(`trd-logout-footnote-brief.md`).
> - **v1.2 (2026-07-16)**: `01_연락처관리_웹서비스_구현요구사항_v1.2.md` §2 FR표·§3-5(FR-14/FR-15, 비밀번호 재설정)와 `04_연락처관리_웹서비스_PRD_v1.3.md` §9 AC-18이 이미 확정되어 있었음에도 본 TRD의 §6/§10/§11 세 곳에서 누락되어 있던 갭을 해소했습니다(`docs/planning/tech-architecture.md` §4/§5 브리프 근거). ① §6 API 설계 요약 표의 인증 그룹에 `POST /auth/find-password`(FR-14)·`PATCH /auth/password`(FR-15) 행을 추가 — 두 API 모두 인증 불필요, 성공 200, 실패 404/422(01 문서 §3-5·§5-1 상태 코드 매트릭스 근거). ② §10-1 `tests/` 파일 구조 제안의 `test_auth.py` 담당 범위에 "FR-14~15 → AC-18"을 추가(신규 파일을 만들지 않고 기존 인증 테스트 파일에 포함 — FR-14/15도 성격상 인증 그룹이므로 파일을 쪼개는 것이 오히려 과잉 분리라고 판단), §10-2 "API 계약" 대상 범위 문구를 "FR-01~13" → "FR-01~15"로 정정. ③ §11 요구사항 추적표에 FR-14~15 행을 추가(대응 섹션: §6 API 설계, §5 인증 설계). 이 갭 패치는 뒤이어 진행되는 06_테스트계획서 작성(qa-planner가 본 TRD의 API 계약을 소스로 사용)이 FR-14/15·AC-18을 빠뜨리지 않게 하기 위한 선행 조치입니다. §6/§10/§11 외 다른 절, 그리고 기존 FR-01~13/AC 매핑 문구는 변경하지 않았습니다(surgical).
...
**커넥션 재연결(`pool_pre_ping=True`, NFR-01)**: `create_engine()`에 `pool_pre_ping=True`를 지정하면, 커넥션 풀이 커넥션을 실제로 꺼내 쓰기 직전에 가벼운 헬스체크(`SELECT 1`)를 실행합니다. DB 컨테이너(`pg-lab`)가 재시작되어 풀에 남아있던 기존 커넥션이 끊어진 상태라면, 이 헬스체크에서 실패를 감지해 해당 커넥션을 버리고 새 커넥션으로 자동 교체한 뒤 요청을 계속 처리합니다 — 별도 코드 없이 SQLAlchemy 내장 동작만으로 이루어집니다. 이 옵션이 없으면 DB 재시작 직후 첫 요청이 죽은 커넥션을 그대로 쓰다가, 처리되지 않은 DB 오류가 `get_db()`의 예외 처리(`rollback()` 후 재던짐)를 그대로 통과해 FastAPI 기본 500 응답으로 노출될 수 있습니다 — 01 문서 §7 NFR-01("어떤 요청에도 서버가 500을 응답하지 않을 것")을 위반하는 경로이므로, `pool_pre_ping=True`는 이 경로 자체를 원천 차단하기 위한 설정입니다.
...
| 인증 | `POST /auth/logout` | 필요[^logout-auth] | 200 | - |
| 인증 | `GET /auth/me` | 필요 | 200 | 401 |
| 인증 | `POST /auth/find-password` | 불필요 | 200 | 404, 422 |
| 인증 | `PATCH /auth/password` | 불필요 | 200 | 404, 422 |
...
[^logout-auth]: 이 칸은 "이 API가 세션/인증 개념을 다루는 그룹에 속한다"는 분류 표시이며, "무세션 호출 시 401을 반환한다"는 뜻이 아니다. 01 문서 §3-1 FR-03에 따라 `POST /auth/logout`은 세션 쿠키 유무와 무관하게 항상 200을 반환한다(로그아웃은 멱등 동작 — "주요 실패 코드" 칸이 처음부터 "-"인 것이 이를 이미 반영함). 상세 근거는 06 테스트계획서 §4-1 `TC-API-AUTH-11` 및 그 각주 `[^logout-idempotent]` 참고.
...
`username`/`new_password` 필드 규칙은 `SignupIn`과 동일한 규칙을 재사용한 것이며(01 문서 §3-5 설계 판단 6 "형식 검증 재사용" 근거), `FindPasswordOut`은 `UserOut`과 달리 `id`를 포함하지 않습니다(01 문서 §3-5 설계 판단 2 근거 — 비인증 상태에서 최소 정보 노출). `MessageOut`은 FR-15 성공 응답(`{"message": "비밀번호가 변경되었습니다"}`)에 대응합니다.
...
```
tests/
├── test_auth.py          # FR-01~04 → AC-01~04, AC-15 / FR-14~15 → AC-18
...
FR-14/15(비밀번호 재설정)는 §2 FR표 기준 "인증" 그룹이고 로그인 불필요 경로라는 점에서 FR-01~04와 성격이 이어지므로, 별도 `test_password_reset.py` 파일을 신설하지 않고 기존 `test_auth.py`에 포함합니다. 파일을 쪼개면 4개였던 테스트 파일 구조가 이유 없이 5개로 늘어나 관리 부담만 커집니다(Simplicity First).

### 10-2. 두 계층의 테스트 도구

| 대상 | 도구 | 이유 |
|---|---|---|
| API 계약 (FR-01~15) | `playwright`의 `APIRequestContext` (`page.request` 또는 `pytest-playwright`의 `request` 픽스처) | 화면 없이 상태 코드·JSON 본문을 직접 검증 — 01 문서의 401/404/409/422 매트릭스 검증에 적합 |
...
| FR-14~15 | §6 API 설계, §5 인증 설계 (로그인 불필요 경로 처리, 비밀번호 변경 시 세션 전체 무효화) |
