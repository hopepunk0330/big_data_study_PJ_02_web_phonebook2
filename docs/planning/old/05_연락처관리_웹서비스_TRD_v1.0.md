...
```mermaid
flowchart TB
    subgraph 화면["화면 계층 (브라우저)"]
        A["static/index.html + app.js"]
    end
    subgraph 표현["표현 계층 (routers/, schemas.py)"]
        B["routers/auth.py"]
...
03 문서(기능정의서)의 구조를 기술 실행 기준으로 확정합니다.
...
`backend/CLAUDE.md`에 명시된 "FastAPI 사용" 방침에 따라 백엔드 코드는 `backend/` 아래에 위치합니다.
