...
- `.claude/skills/*/SKILL.md` — design-concept-round, planning-kickoff-round, ai-image-content-round, report-pdf, review, summary
...
- `docs/harness/**` — claude-harness.md, git-workflow.md, `karpathy_skills.md`(2026-07-29 `docs/karpathy_skills.md`에서 이쪽으로 이동 — `CLAUDE.md`가 `@docs/harness/karpathy_skills.md`로 참조), `harness-versioning.md`(여러 프로젝트에 cp -r로 흩어진 하네스 사본 중 최신 판별 절차 — git 커밋 날짜 비교(1순위) + 포크 로그(2순위), 뒤처진 사본에서 실수로 수정했을 때의 예방·수동 병합 절차 포함. 포크 로그 표는 예시가 아니라 실제 계보 기록이라 리셋 시에도 지우지 않고 그대로 들고 간다, 2026-08-19 신설), `notion-workflow.md`(Notion을 MCP로 연결해 작업할 때의 원칙 — 기존 콘텐츠 절대 편집 금지·항상 새 페이지로 추가, 작업 전 notion-fetch로 상태 확인, 표준 문서 규격(원페이저→PRD→...→회고), 새 프로젝트 시작 시 로컬보다 Notion 확인 우선. 실제 지정 페이지 URL은 프로젝트별 CLAUDE.md에 남기고 이 문서엔 안 적는다, 2026-08-20 신설 — CLAUDE.md의 "테스트 보고서 게시" 절(구 "…Notion 동기화")은 이것과 무관한 별개의 프로젝트 고유 규칙이니 혼동하지 말 것), design-team/figma-file-organization.md, 이 리셋 체크리스트, report-format-guide.md·report-style.css(범용 보고서 HTML/PDF 템플릿 — 프로젝트 브랜드 색이 아니라 남색/호박색 범용 톤이라 포터블. `docs/design/`에 잘못 있던 것을 이번에 이쪽으로 재분류함, 2026-07-16), 그리고 design-team 아래 다음 크래프트/감사 참조 문서들(전부 노드 ID·hex 등 프로젝트 고유 값 미포함, design-systems/graphic-designer/interaction-designer/motion-designer가 만들 때 참조하고 design-qa가 감사할 때 검증 기준으로 삼는 공용 포터블 문서):
...
- 루트 `CLAUDE.md` — **파일 자체를 새 프로젝트로 복사(이식)한 뒤, 아래 "이 프로젝트 고유" 섹션만 지우고 새 프로젝트 내용으로 채우는 "복사 후 리셋" 방식을 따른다 — 빈 파일에서 처음부터 새로 쓰지 않는다**(2026-08-20 정정 — "새로 작성하되 템플릿 참고 가능"이라는 이전 서술이 모호해서, 하네스를 포크할 때마다 이 파일 자체가 이식되지 않고 매번 새로 쓰이는 문제가 반복됐다. 사용자가 직접 지적: "다른 하네스가 자꾸 이 파일을 이식을 안 하네"):
  1. `CLAUDE.md` 파일을 그대로 새 프로젝트 루트에 복사한다.
  2. 아래 "이 프로젝트 고유(지우고 새로 채움)" 목록에 있는 섹션들의 **본문만** 지운다 — 섹션 헤딩 자체는 남긴다.
  3. 아래 "템플릿으로 그대로 재사용(하네스성)" 목록의 섹션들은 **손대지 않고 그대로 둔다.**
  4. 비워둔 섹션에 새 프로젝트의 실제 내용(개요, 코딩 규칙, 검증 명령, 프로젝트 고유 URL 등)을 채운다.
  - **템플릿으로 그대로 재사용(하네스성, 복사 후 손대지 않음)**: `Git 리모트/브랜치 관례`(assignment/origin(JY_Harness) 이중 리모트 패턴 — `git-workflow.md` 요약), `행동 지침`(`@docs/harness/karpathy_skills.md`·`@docs/harness/notion-workflow.md` 참조 패턴, 2026-08-20 후자 추가), `소통 방식`(항상 존댓말 사용 — 전역 `~/.claude/CLAUDE.md`와도 중복, 2026-08-20 신설), `기본 도구`(playwright·context7·스크린샷 캡처 관례 — 전역 `~/.claude/CLAUDE.md`와도 중복), `금지`(원격 push는 명시적 승인 후에만, secrets/.env 폴더 수정 금지 — 프로젝트 고유 규칙이 아니라 항상 지키고 싶은 개인 작업 원칙)
  - **이 프로젝트 고유(지우고 새로 채움)**: `개요`(프로젝트 성격), `코딩 규칙`(들여쓰기·네이밍 컨벤션 — 프로젝트마다 다를 수 있음), `검증 명령`(이 프로젝트의 테스트/린트 경로), `테스트 보고서 게시`(2026-08-20 "…Notion 동기화"에서 개칭)의 구체 URL
...
3. **예시 문장에 프로젝트명·Figma 노드 ID가 하드코딩된 곳**: 프로젝트명(예: "연락처 관리 웹 서비스")보다 **Figma 노드 ID**(예: `259:609`)가 더 조심해야 할 대상이다 — 프로젝트명은 새 프로젝트에서 읽었을 때 "예시구나" 하고 넘어갈 수 있지만, 노드 ID는 새 프로젝트의 Figma 파일에서 완전히 무관한(또는 존재하지 않는) 요소를 가리키므로 그대로 읽으면 혼란을 준다. `.claude/agents/*.md`·`docs/harness/**`(포터블 문서)에 컴포넌트 예시를 들 때는 **노드 ID 없이 이름만** 적는다 — 구체적 ID가 필요하면 `docs/design/design-system.md`(프로젝트 데이터, 매번 새로 채워짐)를 참고하라고 안내한다. — 기능적 로직에는 영향 없지만 새 프로젝트에 그대로 복사하면 예시가 어색해진다. 복사 시점에 프로젝트명만 치환 권장(삭제 대상 아님). **줄 번호는 문서가 수정될 때마다 밀릴 수 있으니, 리셋 실행 시점에 매번 재확인한다(2026-07-17, stale 참조 정정 사례로 아래 항목 갱신):**
   ...
   - `docs/harness/design-team/figma-file-organization.md`의 "연락처" 화면 카테고리 예시(현재 53번째 줄)와 "이 프로젝트(예: 폼이 있는 연락처 관리 서비스)"(2-6번 선제적 기본 구성 판단 기준 예시, 현재 186번째 줄) — 두 곳 모두 "연락처" 예시 언급(2026-07-17 신규 반영, 2026-07-28 이 파일에 0.5절이 확장되며 줄 번호 드리프트 재정정 — 이 파일도 자주 편집되니 줄 번호보다 문구로 찾는 게 안전하다)
   ...
   - `.claude/agents/content-designer.md`의 "제품 화면(로그인, 연락처 관리 등)을 다루지 않는다"(현재 50번째 줄, 2026-07-26 harness-auditor 53차 신규 반영, 63차·66차·68차·69차에서 줄 번호 드리프트 재확인 — 이 파일은 자주 편집되니 줄 번호보다 문구로 찾는 게 안전하다)
...
2. **A그룹에 데이터 흔적이 없는지 grep으로 재확인**(리셋 때마다, 이 체크리스트가 최신인지 검증):
   ```bash
   grep -rl "연락처\|phonebook\|<이 프로젝트의 Figma 파일키>" .claude/agents .claude/skills .claude/commands docs/harness
   ```
