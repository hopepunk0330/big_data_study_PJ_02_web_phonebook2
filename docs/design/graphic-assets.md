...
**⚠ 2026-07-18 각주 추가(harness-auditor 지적, 이력 전파)**: 이 절에서 조정한 BgPixels/ConfettiFooter opacity 값(별 인스턴스 랜덤 불투명도 + 다이아몬드/십자 0.25~0.4 순환 배정, ConfettiFooter 5개/프레임 0.25 통일)은 이후 `docs/design/design-system.md` 40-3절("Auth(`934:2`) 근본 원인 수정 — opacity로 틴트/셰이드 표현 anti-pattern 전면 제거" 라운드, "변환 대상"과 "변환하지 않은 대상" 판단)에서 design-systems가 명시적으로 검토했다 — BgPixels/ConfettiFooter는 "같은 그래픽 요소(별 아이콘 인스턴스/사각형)를 여러 개 배치하며 각기 다른 opacity로 흩뿌려진 색종이(confetti) 느낌의 깊이감을 만드는 장식 스캐터 패턴"이라, 하나의 의미 있는 색상 역할(텍스트/보더/배경 틴트)을 대표하는 Divider/DashedEllipse와 달리 **opacity→독립 Primitive 전환 대상에서 제외**됐다(개별 위치별로 합성 hex를 쪼개면 오히려 토큰 체계를 오염시킨다는 판단, 40-3절 원문). 즉 이 절에서 graphic-designer가 직접 조정한 raw opacity 값들은 이후에도 Primitive 토큰화 대상으로 재작업되지 않고 그대로 유효하다 — 이 절의 조치와 40-3절의 판단은 서로 다른 라운드지만 모순되지 않는다.
...
## 상태: Auth 배경 장식(BgPixels/ConfettiFooter/DashedEllipse) 코드 구현용 좌표·색상 스펙 확정 (2026-07-18, frontend-engineer)

### 배경

사용자가 실 화면(`static/`)에서 로그인/비밀번호재설정 화면 배경을 확인한 결과, `.auth-page`가 단순 `linear-gradient(sky-500, teal-500)`만 갖고 있고 위 두 절이 이미 색상·불투명도까지 확정해 둔 BgPixels(전역 산포)·ConfettiFooter(카드 하단 한 줄) 장식 오브제가 코드에 전혀 구현돼 있지 않음을 발견했다. 색상·불투명도·개수는 이미 문서화돼 있었지만(브랜드가이드 7번, 이 파일의 위 두 절), **코드로 옮기는 데 필요한 실제 좌표(각 오브제의 위치·크기)는 이 문서 어디에도 없었다** — 이번 절이 그 공백을 메운다.

### 실측 (`get_design_context`, login `936:1042`, fileKey `zgGlMBwFglaDlaeyP4CkgR`, 캔버스 1551×963px)

**색상 재확인(에셋 svg 직접 curl, 위 절의 "컬러 자체 오류 정정" 결과가 936:1042에도 반영돼 있음을 재확인)**: BgPixels 다이아몬드/십자 = `#ffffff`(fill 코드에 `bg-white`로 직접 노출), BgPixels 별(`Pixel/Star` 인스턴스) = `#FFCE2C`(`var(--color-amber-600)`와 정확히 일치, svg curl로 `fill="var(--fill-0, #FFCE2C)"` 확인), ConfettiFooter 다이아몬드+별 = `#1A1A1A`(`var(--color-ink-900)`, svg curl로 확인). DashedEllipse stroke는 936:1042 실측 결과 `#71AC9C`인데 이는 raw hex가 아니라 design-system.md 33절/2037행 이하가 이미 등록한 합성 토큰 `color/amber-tint-40-on-sky`(amber/600 40% opacity on sky/500 배경 합성값)의 flatten 결과다 — 코드에서는 flatten hex를 그대로 박지 않고 `var(--color-amber-600)` 보더에 `opacity:.4`를 얹어 동일한 합성을 재현한다(레이어링으로 재현 가능하므로 신규 토큰 불필요).

**BgPixels 14개 좌표(캔버스 1551×963 기준 px, %는 반응형 구현을 위해 변환한 값 — 아래 CSS 구현이 실제로 쓰는 값)**: 다이아몬드는 Figma상 회전 전 정사각형을 감싸는 센터링 프레임(대각선 길이만큼 더 큼)에 담겨 있어, 아래 %는 그 프레임이 아니라 **다이아몬드 중심점**으로 환산했다(중심 = 프레임 top-left + 프레임크기/2). 십자·별은 원본 프레임 자체가 시각 박스와 같아 top-left를 그대로 %로 환산했다.

| 모양 | 원본 node | 원본 px(left,top,size) | 변환 위치(중심 or top-left, %) | opacity |
|---|---|---|---|---|
| 다이아몬드 | `936:1126` | 93.05, 67.13, 14 | 중심 6.6%, 8.0% | 0.4 |
| 별 | `936:1127` | 341.22, 48.14, 14 | top-left 22.0%, 5.0% | 0.4 |
| 십자 | `936:1136` | 620.39, 115.55, 12 | top-left 40.0%, 12.0% | 0.3 |
| 다이아몬드 | `936:1139` | 930.59, 60.48, 9.8 | 중심 60.5%, 7.0% | 0.35 |
| 별 | `936:1140` | 1240.8, 38.52, 18 | top-left 80.0%, 4.0% | 0.3 |
| 십자 | `936:1149` | 1364.88, 173.33, 14 | top-left 88.0%, 18.0% | 0.25 |
| 별 | `936:1152` | 46.52, 365.94, 16 | top-left 3.0%, 38.0% | 0.35 |
| 다이아몬드 | `936:1161` | 1426.91, 472.59, 12.6 | 중심 92.6%, 50.0% | 0.4 |
| 십자 | `936:1162` | 108.56, 597.05, 10 | top-left 7.0%, 62.0% | 0.3 |
| 별 | `936:1165` | 1318.34, 693.36, 14 | top-left 85.0%, 72.0% | 0.35 |
| 다이아몬드 | `936:1174` | 279.17, 762.47, 11.2 | 중심 18.5%, 80.0% | 0.35 |
| 별 | `936:1175` | 775.5, 847.44, 12 | top-left 50.0%, 88.0% | 0.25 |
| 십자 | `936:1184` | 1116.72, 789.66, 12 | top-left 72.0%, 82.0% | 0.25 |
| 다이아몬드 | `936:1187` | 1364.88, 880.01, 8.4 | 중심 88.4%, 92.0% | 0.4 |

개수 검산: 다이아몬드 5(회전된 사각형) + 십자 4(사각형 2개 겹침) + 별 5(`Pixel/Star` 인스턴스) = 14개, 브랜드가이드 7번의 "약 14개"와 정확히 일치.

**DashedEllipse 3개**(카드 주변 장식 점선 원, `color/amber-tint-40-on-sky` 재현):

| node | 원본 px(left,top,size) | 변환 top-left(%) |
|---|---|---|
| `936:1188`(DashedEllipse-0) | 409.9, 360, 22 | 26.4%, 37.4% |
| `936:1189`(DashedEllipse-1) | 1097.8, 618, 40 | 70.8%, 64.2% |
| `936:1190`(DashedEllipse-2) | 1186.8, 315.1, 22 | 76.5%, 32.7% |

**ConfettiFooter 5개**(카드 안쪽 폼 하단, 한 줄, gap 12px, 순서 다이아몬드→별→다이아몬드→별→다이아몬드, 전부 `#1A1A1A` opacity 0.25): 다이아몬드 시각 크기 5.6px(45도 회전), 별 8px — `936:1103`~`936:1124` 실측.

### 코드 적용 원칙 — 이 스펙을 그대로 재사용

원본은 8개 프레임(Join/login/login-알림창/비밀번호재설정 1~성공)마다 BgPixels가 **독립 복제된 raw 좌표**라 프레임마다 배치가 미묘하게 다르지만(위 "935:33 공통 배경 관련 확인" 절 참고), 이 좌표를 그대로 코드에 옮기면 화면마다 다른 마크업을 3벌 유지보수해야 해 과잉이라고 판단했다 — dev-pl 지시("`.auth-page`를 사용하는 모든 화면에 일관되게 적용")에 따라 login(`936:1042`) 1개 프레임의 좌표만 실측해 **모든 `.auth-page` 화면(로그인/회원가입 토글, 비밀번호재설정 1·2단계)에 동일하게 재사용**한다 — 이는 임의 근사치가 아니라 dev-pl이 명시적으로 지시한 단순화 범위다.

### 배경색 자체 결함 발견(이번 브리프 범위, 함께 수정)

`936:1042`의 배경은 `bg-[var(--color-bg-brand-blue,#1395e6)]`(단색 sky/500 100% 풀블리드)이지, 코드에 있던 `linear-gradient(sky-500, teal-500)`가 아니다. `brand-guide.md` 48행("Join/login/login-알림창 풀블리드 배경(전체 100%)")과 design-system.md 1734행("Primary, 화면 전체 면적을 지배하는 구조적 배경색")도 단색임을 명시한다 — 그라디언트는 이번에 근거 없는 값으로 확인돼 함께 단색으로 정정한다.

**값 출처**: `get_design_context`(login `936:1042`, fileKey `zgGlMBwFglaDlaeyP4CkgR`) 실측 좌표/색상, 개별 svg 에셋 curl(별 `#FFCE2C`, 컨페티 다이아몬드/별 `#1A1A1A`, DashedEllipse stroke `#71AC9C`), `docs/design/brand-guide.md` 48/120행, `docs/design/design-system.md` 33절/1734행/2037행 이하(합성 토큰 근거) — 임의 근사치 없음(단, 위 "코드 적용 원칙" 절에 명시한 대로 8프레임 개별 좌표 대신 login 1개 프레임 좌표를 3개 화면에 공용 재사용하는 단순화는 dev-pl 지시에 따른 의도된 근사).
