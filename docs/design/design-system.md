...
**⚠ 2026-07-15 문서 복구 메모(투명성 기록)**: 이 세션 중 서브에이전트가 "파일 끝에 새 절만 append" 지시를 잘못 수행(전체 덮어쓰기)해 파일이 586줄→61줄로 손상되는 사고가 있었고, 이후 복구 과정에서도 한 차례 더 잘못된 번호(0-9/0-10)로 다른 내용이 섞여 들어가는 2차 혼선이 있었다. git 커밋(`4a0db75`, 520줄, 0-1~0-8절/1~9-5절)으로 대부분 복구했고, 커밋되지 않았던 실제 0-9/0-10절은 design-pl이 사고 직전 직접 읽어뒀던 원문으로 복원했다. 단, 0-9/0-10절이 2/3/5/7-2절에 반영했던 세부 표 갱신 일부는 git 이력에 없어 완전 복구가 어려웠다 — 알려진 사실은 아래 각 절에 반영했지만, **design-systems가 다음 라운드에서 Figma를 직접 재조회해 2/3/5/7-2절을 라이브 상태와 대조·재확정하는 후속 검증이 필요하다**(Figma 파일 자체는 이 사고로 전혀 영향받지 않았다 — 문서만 손상됐었다).
...
**Spacing(`95:4`, 루트 `95:162`)** — 기존 legacy spacing(1~8)/radius(sm/md/lg/full)는 무수정. 1-1절에 신규 등록됐던 `radius/none`(0)/`radius/6`(6)/`radius/10`(10)와 `border/hairline`(1)/`border/base`(2)/`border/heavy`(3)가 이 페이지 카탈로그에는 반영돼 있지 않아 신규 섹션 2개(Radius/Border)로 추가.
...
| TypeSelector | Badge `102:3` | `257:28` | Category × State=Selected/Unselected (8) × Focus=No/Yes(9-3절, 16 variant). **0-9절**: Selected 4종은 CatBadge 토큰 공유(2절 참고), Unselected는 전용 회색 토큰 유지. | `343:1146` |
| Count Pill | Sidebar Nav Item `103:92` | `258:16` | State=Active(흰 배경, 그림자 없음)/Inactive(앰버 배경, Shadow/Hard-1). | (스펙 시트 없음) |
| Sidebar Nav Item | Sidebar Nav Item `103:92` | `258:29` | State=Active/Inactive × Focus=No/Yes(9-3절, 4 variant). **9-5절**: Inactive+Focus=Yes(`287:17`)는 3px ink OUTSIDE 스트로크로 링 구현. | `343:1106` |
...
| NeoInput | Input `100:2` | `288:12` | Focus=No/Yes(9-3절) × Error=No/Yes(**0-10절**, 4 variant) × Content=Placeholder/Filled(**0-11/0-12절**, 실사용 매트릭스 7 variant, Placeholder×Focus=Yes×Error=Yes 제외). Placeholder 텍스트=`color/text-placeholder-input`(#BBBBBB, NeoInput 전용, 0-12절). | `344:721` |
| CornerInput | Input `100:2` | `288:27` | Focus=No/Yes(9-3절) × Error=No/Yes(**0-10절**, 4 variant) × Content=Placeholder/Filled(**0-11절**, 실사용 매트릭스 7 variant). **0-3절**: 모서리 CornerBracket 제거, 순수 2px ink 보더. Placeholder 텍스트=`color/text-placeholder`(#CCCCCC). 베이스 폭 392px. | `344:740` |
...
- **Focus** — `DROP_SHADOW`, offset (0,0), blur 0, spread 3px, color `#1A1A1A` 100%. **예외 1건(9-5절)**: Sidebar Nav Item Inactive+Focus=Yes는 3px ink OUTSIDE 스트로크로 대체.
...
- 포커스 링(spread 3px ink)은 컴포넌트 로컬 raw 값 — design-systems가 필요 시 `component/focus-ring-*` 토큰으로 승격 검토 가능.
